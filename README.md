...
within the assessment's stated 8–12 hour scope. Setup instructions, architecture,
design decisions, verification results, and known limitations are documented below.
...
identities — there is no password, registration, or user store. This keeps the demo
login simple while exercising JWT validation and role-based authorization.
...
The upload endpoint uses a Minimal API to keep multipart streaming explicit; other
endpoints use controllers. See [Streaming approach](#streaming-approach) for the
upload implementation and its design rationale.
...
Tags are normalized and deduplicated, then stored in a boxed-delimiter column to
support exact tag matches without matching substrings.
...
- The upload endpoint uses a Minimal API as a simplicity preference: it reads the
  multipart request body directly without MVC form model binding. Streaming is also
  possible in an MVC controller when configured to avoid consuming the form body
  before the action runs.

## Database/filesystem consistency strategy

Files are written to a temporary path and committed with an atomic rename. If the
database write then fails, synchronous compensation deletes the committed file. A
crash window remains if the process dies between the filesystem commit and the
database write: SQL Server and the filesystem do not share a transaction. Recovery
from that window uses the `--reconcile` command described next.
...
implementation: a code review found that
...
- Cross-user access returns 404 to avoid confirming another user's file exists.
  A non-admin request to the hard-delete endpoint returns 403 because that endpoint
...
Additional verification included `curl`-driven smoke tests of
...
401) during development, and a browser-based visual check of the login page, the
...
visual inspection session, which could not complete the native `confirm()` dialog.
Playwright handles this dialog via its `dialog` event.
...
- The directly navigable preview page retrieves metadata through
  `GET /api/files/{id}`, a supporting endpoint in addition to the assessment's
...
- **Docker remains an outstanding verification item.** Rechecked 2026-09-17: the Docker
  ...
  ```
- **The DB/filesystem crash-window gap** described in
  [Database/filesystem consistency strategy](#databasefilesystem-consistency-strategy)
  has a recovery mechanism — the `--reconcile` administrative command — but it is
  deliberately on-demand, not a
...
- Scheduling `--reconcile` (a cron entry, a scheduled task, or a lightweight
  ...
  on a timer) so DB/filesystem inconsistencies are detected proactively rather than
  only when an operator remembers to run it.
